...
def coverage(model, data: Union[Data, List[Data]]) -> List[Union[torch.Tensor, Data]]:
    ...
    for d in tqdm(data):
        ...
        fine_preds = np.array([event_pred.detach().numpy() for event_pred in fine_pred_chunk])
    ...
    truth_directions = torch.stack((truth_x, truth_y, truth_z), dim=1)
    truth_x_list = []
    for d in data:
        x = model.backbone(d)
        truth_x_list.extend(x)
    x = torch.stack(truth_x_list)
    x = torch.cat([x,truth_directions],dim=1).float()
    x = model._discriminator(x)
