...
def predict_likelihood(
    model, zen, azi, delta, data: Union[Data, List[Data]]
    ...
    directions = torch.stack(
        (torch.tensor(x), torch.tensor(y), torch.tensor(z)), dim=1
    )
    ...
    x = torch.cat(
        [x, y], dim=1
    ).float()  # Shape: (num_events * npix, feature_dim + 3)
    ...
    preds = np.array(
        [event_pred.detach().numpy() for event_pred in pred_chunk]
    )
...
def rotate_ze_az(zenith, azimuth, truth_ze, truth_az):
    x = np.sin(zenith) * np.cos(azimuth)
    y = np.sin(zenith) * np.sin(azimuth)
    z = np.cos(zenith)
    x, y, z = rotate_around_axis(x, y, z, "z", -truth_az)
    x, y, z = rotate_around_axis(x, y, z, "y", -(truth_ze - np.pi / 2))
    zenith = np.arccos(z) - np.pi / 2
    azimuth = np.arctan2(y, x)
    return zenith, azimuth


spline_ze, spline_az = rotate_ze_az(spline_ze, spline_az, truth_ze, truth_az)
...
ax.plot(
    np.degrees(spline_az),
    np.degrees(spline_ze),
    "gx",
    markersize=10,
    label="Spline MPE",
)
ax.plot(
    np.degrees(max_llh_az),
    np.degrees(max_llh_ze),
    "bx",
    markersize=10,
...
contour_line = Line2D(
    [0],
    [0],
    color=cs.collections[0].get_edgecolor()[0],
    linewidth=1,
    linestyle="solid",
)
