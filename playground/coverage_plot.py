...
plt.figure()
plt.step(chi2,np.arange(len(sorted_delta_log))/len(sorted_delta_log),where = 'post', linewidth = 2.5 ,label = 'SBI model coverage')
...
plt.xlim(0,1)
plt.ylim(0,1)
plt.text(.2, .7, r'Conservative', ha='left', va='top',style = 'italic', transform=plt.gca().transAxes)
plt.text(.8, .3, r'Overconfident', ha='right', va='bottom',style = 'italic', transform=plt.gca().transAxes)
plt.legend()
